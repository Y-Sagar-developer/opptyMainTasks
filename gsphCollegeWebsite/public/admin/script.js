...
(function () {
  ...
  function saveStudentAttendanceCache(roll, data) {
    roll = (roll || "").trim().toUpperCase();
    if (!roll) return;
    
    // Save individual student attendance cache
    var cacheData = {
      dailyAttendance: (data.dailyAttendance && data.dailyAttendance[roll]) || {},
      attendanceMetadata: (data.attendanceMetadata && data.attendanceMetadata[roll]) || {},
      lastAttendanceUpdate: data.lastAttendanceUpdate || null,
      syncTimestamp: new Date().toISOString()
    };
    
    localStorage.setItem("cms_daily_attendance_" + roll, JSON.stringify(cacheData));
  
    // Also update global sync marker for all students
    localStorage.setItem("cms_attendance_sync_time", new Date().toISOString());
    
    // Trigger storage event for cross-tab sync
    try {
      window.dispatchEvent(new StorageEvent('storage', {
        key: "cms_daily_attendance_" + roll,
  ...
  function syncDailyAttendanceSummary(roll, data) {
    ...
    // Check if student has subjects array (might not exist for MySQL loaded students)
  ...
  async function renderStudentsTable(filterBranch, search) {
    var tb = $("#students-table tbody");
    if (!tb) return;
    tb.innerHTML = "<tr><td colspan='8' style='text-align:center;'>Loading students...</td></tr>";
    
    try {
      // Fetch students from MySQL database
      const response = await getAllStudents(filterBranch || null);
      
      if (!response.success) {
        tb.innerHTML = "<tr><td colspan='8' style='text-align:center; color:red;'>Failed to load students</td></tr>";
        return;
      }
      ...
      // Normalize MySQL data to match localStorage format
      students = students.map(function(s) {
        return {
          roll: s.roll,
          name: s.name,
          branch: s.branch_code || s.branch,
          branch_code: s.branch_code || s.branch,
          year: s.year,
          section: s.section,
          password: s.password,
          email: s.email,
          phone: s.phone,
          cgpa: s.cgpa,
          feeStatus: s.fee_status || s.feeStatus,
          fee_status: s.fee_status || s.feeStatus,
          photo: s.photo || avatarUrl(s.roll),
          subjects: s.subjects || [], // Initialize empty subjects array
          gpaSem: s.gpaSem || "0.00",
          attendancePct: s.attendancePct || 80
        };
      });
      
      // Update localStorage with fetched data for compatibility
      ...
      rows.forEach(function (s) {
        ...
        tr.innerHTML =
          ...
          escapeHtml(s.name) +
          "</td><td>" +
          escapeHtml(s.branch_code || s.branch) +
          "</td><td>" +
          s.year +
          "</td><td>" +
          ...
          "</td><td>" +
          escapeHtml(s.fee_status || s.feeStatus) +
          "</td><td>" +
      ...
      tb.innerHTML = "<tr><td colspan='8' style='text-align:center; color:red;'>Error: " + error.message + "</td></tr>";
    }
  }
  ...
  function initAdminDashboard() {
    ...
    $("#students-table").onclick = function (e) {
      ...
      if (del) {
        ...
        deleteStudent(del).then(function(response) {
          if (response.success) {
            toast("Student removed successfully!", "success");
            rerenderStudents();
            refreshCharts();
          } else {
            toast("Failed to delete student: " + (response.message || "Unknown error"), "error");
          }
    ...
    $("#form-student-modal").onsubmit = async function (e) {
      e.preventDefault();
      data = ensureAppData();
      ...
      // Prepare payload for MySQL (using snake_case field names)
      var payload = {
        roll: newRoll,
        name: $("#sm-name").value.trim(),
        branch_code: $("#sm-branch").value,
      ...
      try {
        if (!oldRoll) {
          ...
          if (response.success) {
            ...
            await rerenderStudents();
            refreshCharts();
          } else {
            toast("Failed to add student: " + (response.message || "Unknown error"), "error");
          }
        } else {
          // Update existing student in MySQL
          if (oldRoll !== newRoll) {
            toast("Roll number cannot be changed. Delete and create new student instead.", "error");
            return;
          }
          
          const response = await updateStudent(oldRoll, payload);
          ...
          if (response.success) {
            ...
            toast("Failed to update student: " + (response.message || "Unknown error"), "error");
          }
        }
      } catch (error) {
    ...
    async function renderTeachers() {
      ...
      tb.innerHTML = "<tr><td colspan='4' style='text-align:center;'>Loading teachers...</td></tr>";
        ...
        if (response.success) {
          ...
          teachers.forEach(function (t) {
            ...
            tr.innerHTML =
              ...
              escapeHtml(t.role) +
              "</td><td>" +
              escapeHtml(t.branch_code || t.branch || 'N/A') +
              "</td>";
            tb.appendChild(tr);
          });
        } else {
          tb.innerHTML = "<tr><td colspan='4' style='text-align:center; color:red;'>Failed to load teachers</td></tr>";
        }
      } catch (error) {
        console.error('Error loading teachers:', error);
        tb.innerHTML = "<tr><td colspan='4' style='text-align:center; color:red;'>Error: " + error.message + "</td></tr>";
      }
    }
    renderTeachers();
    ...
    $("#form-teacher-modal").onsubmit = async function (e) {
      e.preventDefault();
      ...
      try {
        ...
        if (response.success) {
          toast("Teacher account created successfully!", "success");
          closeModal("modal-teacher");
          await renderTeachers();
        } else {
          toast("Failed to add teacher: " + (response.message || "Unknown error"), "error");
        }
      } catch (error) {
          ...
          $("#branches-list").innerHTML = branches
            .map(function (b) {
              return (
                ...
                escapeHtml(b.code) +
                "</strong><p style='margin:0.35rem 0 0;color:var(--text-muted);font-size:0.9rem'>" +
                escapeHtml(b.name) +
                "</p></div></div>"
              );
            })
    ...
    renderBranches();
    $("#btn-add-branch").onclick = async function () {
      var code = $("#new-branch-input").value.trim().toUpperCase();
      var name = $("#new-branch-name-input") ? $("#new-branch-name-input").value.trim() : code;
      
      if (!code) {
        toast("Please enter branch code", "error");
        return;
      }
      
      try {
        const response = await addBranch({ code: code, name: name });
        
        if (response.success) {
          toast("Branch added successfully!", "success");
          $("#new-branch-input").value = "";
          if ($("#new-branch-name-input")) $("#new-branch-name-input").value = "";
          await renderBranches();
          populateBranchSelects(fb);
          fb.innerHTML = '<option value="">All branches</option>';
          
          // Reload branches for dropdown
          const branchesResp = await getAllBranches();
          if (branchesResp.success) {
            branchesResp.branches.forEach(function (b) {
              var o = document.createElement("option");
              o.value = b.code;
          ...
          toast("Failed to add branch: " + (response.message || "Unknown error"), "error");
        }
      } catch (error) {
    ...
    async function renderSubjects() {
      ...
      tb.innerHTML = "<tr><td colspan='4' style='text-align:center;'>Loading subjects...</td></tr>";
      ...
      try {
        ...
        if (response.success) {
          ...
          tb.innerHTML = "<tr><td colspan='4' style='text-align:center; color:red;'>Failed to load subjects</td></tr>";
        }
      } catch (error) {
    ...
    $("#form-subject-modal").onsubmit = async function (e) {
      e.preventDefault();
      
      var subjectData = {
        code: $("#sub-code").value.trim().toUpperCase(),
        name: $("#sub-name").value.trim(),
        branch_code: $("#sub-branch").value,
      ...
      try {
        ...
        if (response.success) {
          ...
          toast("Failed to add subject: " + (response.message || "Unknown error"), "error");
        }
      } catch (error) {
    ...
    var currentAttMonth = new Date().getMonth();
    var currentAttYear = new Date().getFullYear();
    var currentAttStudent = null;
    
    $("#btn-search-student-att").onclick = function() {
      var roll = $("#att-student-roll").value.trim().toUpperCase();
      if (!roll) {
        toast("Please enter a roll number", "error");
        return;
      }
      
      data = ensureAppData();
      var student = data.students.find(function(s) { return s.roll === roll; });
      ...
      currentAttStudent = student;
      currentAttMonth = new Date().getMonth();
      currentAttYear = new Date().getFullYear();
      
      // Show student info
      $("#att-student-photo").src = student.photo || avatarUrl(student.roll);
      $("#att-student-name").textContent = student.name;
      $("#att-student-roll-display").textContent = student.roll;
      $("#att-student-branch").textContent = student.branch;
      $("#att-student-year").textContent = "Year " + student.year;
      $("#att-student-section").textContent = "Sec " + student.section;
      
      $("#student-attendance-panel").style.display = "block";
      
      // Load attendance from MySQL
      loadStudentAttendanceFromDB(student.roll).then(function() {
        renderAdminAttendanceCalendar();
        toast("Loaded attendance for " + student.name, "success");
      });
    };
    
    $("#btn-prev-month").onclick = function() {
      if (!currentAttStudent) return;
      currentAttMonth--;
      if (currentAttMonth < 0) {
        currentAttMonth = 11;
        currentAttYear--;
      }
      renderAdminAttendanceCalendar();
    ...
    // Load student attendance from MySQL database
    async function loadStudentAttendanceFromDB(roll) {
      try {
        const response = await getAttendance(roll);
        
        if (response.success) {
          var attendanceRecords = response.attendance || [];
          
          // Update localStorage with MySQL data
          data = ensureAppData();
          data.dailyAttendance = data.dailyAttendance || {};
          ...
          // Convert MySQL records to localStorage format
          attendanceRecords.forEach(function(record) {
            var dateKey = record.date; // Already in YYYY-MM-DD format
            data.dailyAttendance[roll][dateKey] = record.status;
            data.attendanceMetadata[roll][dateKey] = {
              status: record.status,
              markedBy: record.marked_by,
              markedOn: record.marked_at,
              approvedBy: record.marked_by,
              approvedOn: record.marked_at
            };
          });
          
          saveApp(data);
          syncDailyAttendanceSummary(roll, data);
        }
      } catch (error) {
        console.error('Error loading attendance from database:', error);
        toast("Failed to load attendance from database", "error");
      }
    }
    
    $("#btn-next-month").onclick = function() {
      if (!currentAttStudent) return;
      currentAttMonth++;
      if (currentAttMonth > 11) {
        currentAttMonth = 0;
        currentAttYear++;
      }
      renderAdminAttendanceCalendar();
    ...
    function renderAdminAttendanceCalendar() {
      ...
      data = ensureAppData();
      var cal = $("#admin-attendance-calendar");
      if (!cal) return;
      
      var monthNames = ["January", "February", "March", "April", "May", "June",
                        "July", "August", "September", "October", "November", "December"];
      $("#current-month-display").textContent = monthNames[currentAttMonth] + " " + currentAttYear;
      
      var firstDay = new Date(currentAttYear, currentAttMonth, 1).getDay();
      var daysInMonth = new Date(currentAttYear, currentAttMonth + 1, 0).getDate();
      
      // Initialize attendance data structure
      if (!data.dailyAttendance) data.dailyAttendance = {};
      if (!data.dailyAttendance[currentAttStudent.roll]) {
        data.dailyAttendance[currentAttStudent.roll] = {};
      }
      if (!data.attendanceMetadata) data.attendanceMetadata = {};
      if (!data.attendanceMetadata[currentAttStudent.roll]) {
        data.attendanceMetadata[currentAttStudent.roll] = {};
      }
      ...
      for (var day = 1; day <= daysInMonth; day++) {
        var dateKey = currentAttYear + "-" + String(currentAttMonth + 1).padStart(2, "0") + "-" + String(day).padStart(2, "0");
        var dayOfWeek = new Date(currentAttYear, currentAttMonth, day).getDay();
        var isWeekend = (dayOfWeek === 0 || dayOfWeek === 6);
        var holiday = attendanceHoliday(currentAttMonth + 1, day);
        var isHoliday = isWeekend || !!holiday;
        var isFuture = new Date(currentAttYear, currentAttMonth, day) > new Date();
        
        var status = data.dailyAttendance[currentAttStudent.roll][dateKey] || "unmarked";
        ...
        var today = new Date();
        if (day === today.getDate() && currentAttMonth === today.getMonth() && currentAttYear === today.getFullYear()) {
          classes += " today";
        }
        
        var holidayStyle = holiday && holiday.image
          ? ' style="cursor: pointer; background-image: linear-gradient(to top, rgba(0,0,0,0.72), rgba(0,0,0,0.18)), url(\'' + holiday.image + '\')"'
          : ' style="cursor: pointer;"';
        html += '<div class="' + classes + '" data-date="' + dateKey + '" data-day="' + day + '"' + holidayStyle + '>';
        html += '<div class="calendar-day-number">' + day + '</div>';
        if (status === "holiday") {
          html += '<div class="calendar-day-status">' + escapeHtml(holiday ? holiday.name : "Holiday") + '</div>';
        } else if (status !== "future" && status !== "unmarked") {
      ...
      cal.innerHTML = html;
      renderHolidayYearOverview("admin-holiday-year-list", "admin-holiday-count");
      
      // Update summary
      var percentage = workingDays > 0 ? Math.round((presentCount / workingDays) * 100) : 0;
      $("#att-present-count").textContent = presentCount;
      $("#att-absent-count").textContent = absentCount;
      $("#att-working-days").textContent = workingDays;
      $("#att-month-percentage").textContent = percentage + "%";
      $("#att-student-percentage").textContent = percentage + "%";
      
      // Add click handlers to open modal
      cal.querySelectorAll(".calendar-day[data-date]").forEach(function(dayEl) {
        dayEl.onclick = function() {
          var dateKey = this.getAttribute("data-date");
          var dayOfWeek = new Date(dateKey).getDay();
          var isWeekend = (dayOfWeek === 0 || dayOfWeek === 6);
          var dateParts = dateKey.split("-");
          var holiday = attendanceHoliday(Number(dateParts[1]), Number(dateParts[2]));
          var isHoliday = isWeekend || !!holiday;
          var isFuture = new Date(dateKey) > new Date();
          
          if (isHoliday || isFuture) {
            if (isHoliday) {
              toast("Holiday: " + (holiday ? holiday.name : "Weekend"), "info");
            } else {
              toast("Cannot mark attendance for future dates", "info");
            }
            return;
          }
          
          // Open attendance marking modal
          openAttendanceModal(dateKey);
        };
      });
    ...
    var currentMarkingDate = null;
    
    function openAttendanceModal(dateKey) {
      if (!currentAttStudent) return;
      
      currentMarkingDate = dateKey;
      ...
      $("#att-modal-student-name").textContent = currentAttStudent.name + " (" + currentAttStudent.roll + ")";
      $("#att-modal-date").textContent = formattedDate;
      $("#att-modal-day").textContent = dayNames[date.getDay()];
      
      $("#modal-mark-attendance").classList.add("open");
    }
    
    $("#btn-mark-present").onclick = function() {
      markAttendance("present");
    };
    
    $("#btn-mark-absent").onclick = function() {
      markAttendance("absent");
    };
    
    $("#btn-clear-attendance").onclick = function() {
      markAttendance("unmarked");
    };
    
    async function markAttendance(status) {
      if (!currentAttStudent || !currentMarkingDate) return;
      
      var session = getAdminSession() || { name: "Admin", role: "admin" };
      
      try {
        // Prepare attendance data for MySQL
        // Note: Using 'GENERAL' as subject_code for daily attendance
        var attendanceData = {
          student_roll: currentAttStudent.roll,
          subject_code: 'GENERAL', // Default subject for daily attendance
          date: currentMarkingDate,
          status: status === "unmarked" ? "absent" : status,
          marked_by: session.name + " (" + session.role + ")"
        };
        
        if (status === "unmarked") {
          // For now, we'll mark as absent instead of deleting
          toast("Attendance cleared for " + currentMarkingDate, "info");
        } else {
          // Mark attendance in MySQL using the API
          ...
          if (response.success) {
            ...
            toast("Marked " + statusText + " for " + currentMarkingDate, "success");
          } else {
            toast("Failed to mark attendance: " + (response.message || "Unknown error"), "error");
            return;
          }
        }
        ...
        data = ensureAppData();
        data.dailyAttendance = data.dailyAttendance || {};
        data.dailyAttendance[currentAttStudent.roll] = data.dailyAttendance[currentAttStudent.roll] || {};
        data.attendanceMetadata = data.attendanceMetadata || {};
        data.attendanceMetadata[currentAttStudent.roll] = data.attendanceMetadata[currentAttStudent.roll] || {};
        
        if (status === "unmarked") {
          delete data.dailyAttendance[currentAttStudent.roll][currentMarkingDate];
          if (data.attendanceMetadata && data.attendanceMetadata[currentAttStudent.roll]) {
            delete data.attendanceMetadata[currentAttStudent.roll][currentMarkingDate];
          }
        } else {
          data.dailyAttendance[currentAttStudent.roll][currentMarkingDate] = status;
          data.attendanceMetadata[currentAttStudent.roll][currentMarkingDate] = {
            status: status,
            markedBy: session.name + " (" + session.role + ")",
        ...
        syncDailyAttendanceSummary(currentAttStudent.roll, data);
        data.lastAttendanceUpdate = {
          roll: currentAttStudent.roll,
          date: currentMarkingDate,
          status: status,
          updatedAt: new Date().toISOString()
        };
        saveApp(data);
        saveStudentAttendanceCache(currentAttStudent.roll, data);
        localStorage.setItem("cms_last_attendance_update", JSON.stringify(data.lastAttendanceUpdate));
        currentAttStudent = findStudent(currentAttStudent.roll) || currentAttStudent;
        renderAdminAttendanceCalendar();
        refreshAdminCounters();
        if (typeof refreshCharts === "function") refreshCharts();
        $("#modal-mark-attendance").classList.remove("open");
      } catch (error) {
    ...
    function renderAttendanceRecordsTable() {
      ...
      data = ensureAppData();
      var tbody = $("#attendance-records-table tbody");
      if (!tbody) return;
      
      tbody.innerHTML = "";
      
      if (!data.dailyAttendance || !data.dailyAttendance[currentAttStudent.roll]) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; color: var(--text-muted);">No attendance records found</td></tr>';
        return;
      }
      
      var records = [];
      var attendanceData = data.dailyAttendance[currentAttStudent.roll];
      var metadataData = data.attendanceMetadata && data.attendanceMetadata[currentAttStudent.roll] || {};
      
      // Collect all records
      Object.keys(attendanceData).forEach(function(dateKey) {
        var status = attendanceData[dateKey];
        if (status !== "unmarked" && status !== "future") {
          var metadata = metadataData[dateKey] || {};
          var date = new Date(dateKey);
          var dayNames = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
          ...
          records.push({
            ...
            status: status,
            markedBy: metadata.markedBy || "System",
            markedOn: metadata.markedOn ? new Date(metadata.markedOn).toLocaleString() : "-"
          });
        }
      });
      ...
      if (records.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; color: var(--text-muted);">No attendance marked yet</td></tr>';
        return;
      }
      ...
      records.forEach(function(record) {
        ...
        var statusBadge = record.status === "present" 
          ? '<span class="tag" style="background: var(--success); color: white;">✓ Present</span>'
          : '<span class="tag" style="background: var(--danger); color: white;">✗ Absent</span>';
        
        tr.innerHTML = 
          '<td><strong>' + escapeHtml(record.date) + '</strong></td>' +
          '<td>' + escapeHtml(record.day) + '</td>' +
          '<td>' + statusBadge + '</td>' +
          '<td>' + escapeHtml(record.markedBy) + '</td>' +
          '<td style="font-size: 0.85rem;">' + escapeHtml(record.markedOn) + '</td>' +
          '<td><button class="btn btn-outline btn-sm" data-delete-date="' + escapeHtml(record.date) + '">Delete</button></td>';
        
        tbody.appendChild(tr);
      });
      
      // Add delete handlers
      tbody.querySelectorAll("[data-delete-date]").forEach(function(btn) {
        btn.onclick = function() {
          var dateKey = this.getAttribute("data-delete-date");
          if (confirm("Delete attendance record for " + dateKey + "?")) {
            delete data.dailyAttendance[currentAttStudent.roll][dateKey];
            if (data.attendanceMetadata && data.attendanceMetadata[currentAttStudent.roll]) {
              delete data.attendanceMetadata[currentAttStudent.roll][dateKey];
            }
            syncDailyAttendanceSummary(currentAttStudent.roll, data);
            data.lastAttendanceUpdate = {
              roll: currentAttStudent.roll,
    ...
    $("#form-marks").onsubmit = async function (e) {
      e.preventDefault();
      ...
      if (!roll || !sub) {
        toast("Please select student and subject", "error");
        return;
      }
      
      try {
        // Prepare marks data for MySQL
        var marksData = {
          roll: roll,
          subject_code: sub,
          internal_marks: im !== "" ? Number(im) : null,
          semester_marks: sm !== "" ? Number(sm) : null,
          exam_type: 'regular'
        };
        
        const response = await addOrUpdateMarks(marksData);
        ...
        if (response.success) {
          ...
          toast("Failed to save marks: " + (response.message || "Unknown error"), "error");
        }
      } catch (error) {
    ...
    $("#form-assignment").onsubmit = async function (e) {
      e.preventDefault();
      
      var assignmentData = {
        title: $("#as-title").value.trim(),
        branch_code: $("#as-branch").value,
        subject_code: $("#as-subject").value.trim().toUpperCase(),
        due_date: $("#as-due").value,
      ...
      try {
        ...
        if (response.success) {
          toast("Assignment posted successfully!", "success");
          $("#form-assignment").reset();
          await renderAssignmentsAdmin();
        } else {
          toast("Failed to post assignment: " + (response.message || "Unknown error"), "error");
        }
      } catch (error) {
    ...
    async function renderAssignmentsAdmin() {
      ...
      tb.innerHTML = "<tr><td colspan='4' style='text-align:center;'>Loading assignments...</td></tr>";
        ...
        if (response.success) {
          ...
          assignments.forEach(function (a) {
            ...
            tr.innerHTML =
              ...
              escapeHtml(a.title) +
              "</td><td>" +
              escapeHtml(a.branch_code || a.branch) +
              "</td><td>" +
              escapeHtml(a.subject_code || a.subject) +
              "</td><td>" +
              escapeHtml(a.due_date || a.due) +
              "</td>";
            tb.appendChild(tr);
          });
        } else {
          tb.innerHTML = "<tr><td colspan='4' style='text-align:center; color:red;'>Failed to load assignments</td></tr>";
        }
      } catch (error) {
    ...
    $("#form-notification").onsubmit = async function (e) {
      e.preventDefault();
      
      var notificationData = {
        title: $("#nt-title").value.trim(),
        message: $("#nt-body").value.trim(),
        audience: $("#nt-audience").value,
        branch_code: $("#nt-branch").value || null,
      };
      ...
      try {
        ...
        if (response.success) {
          toast("Notification published successfully!", "success");
          $("#form-notification").reset();
          await renderNotifsAdmin();
        } else {
          toast("Failed to publish notification: " + (response.message || "Unknown error"), "error");
        }
      } catch (error) {
    ...
    async function renderNotifsAdmin() {
      var ul = $("#notifications-admin");
      ul.innerHTML = "<li style='text-align:center;'>Loading notifications...</li>";
      ...
      try {
        ...
        if (response.success) {
          ...
          if (notifications.length === 0) {
            ul.innerHTML = "<li style='text-align:center; color:var(--text-muted);'>No notifications yet</li>";
            return;
          }
          
          ul.innerHTML = notifications
            .map(function (n) {
              return (
                "<li><strong>" +
                escapeHtml(n.title) +
                "</strong><div class='date'>" +
                escapeHtml(n.created_at ? n.created_at.slice(0, 10) : n.date) +
                " · " +
          ...
          ul.innerHTML = "<li style='text-align:center; color:red;'>Failed to load notifications</li>";
        }
      } catch (error) {
        console.error('Error loading notifications:', error);
        ul.innerHTML = "<li style='text-align:center; color:red;'>Error: " + error.message + "</li>";
      }
    }
    renderNotifsAdmin();
